...
边界即消失，默认）、**环面世界**（边缘环绕）或**无限世界**（无界、稀疏存储，用鼠标拖动
平移查看）。画布大小、世界类型等参数会在多次运行之间被记住。
...
- **无限** —— 没有墙的无界世界。用**稀疏方式**存储（只保留活细胞，放在一个哈希
  ...
  永远飞下去。终端显示的是这个世界的一个**视口**，用鼠标拖动来移动它（见下）。
...
在无限世界里画布不再有固定尺寸，所以你用**按住鼠标左键拖动**来平移视口（抓取式：
光标下的点保持不动）。`Tab`/`←`/`→` 则和有界世界里一样，用来移动按钮选择。状态栏
显示摄像机位置 `Cam: (x,y)` 和存活细胞数。编辑模式下用方向键让光标在无界世界里自由
移动，视口会跟随它。从无限切回有界世界时，会把当前视口作为新的有限画布。
