...
图形（真实像素，而非文本字符）绘制，界面下方有按钮栏。世界可以是**有限世界**（图案飞出
...
棋盘以真实的 **Sixel** 位图绘制：每个细胞变成一块像素，于是画布受限于终端的**像素数**，
而不是字符网格，世界可以做到几百格宽。图像会自动缩放（zoom-to-fit）：画布小的时候
...
在无限世界里画布不再有固定尺寸，所以普通模式下**方向键用来平移视口**，`Tab` 切换
按钮（有界世界里方向键仍兼作按钮选择，因为没什么可平移的）。状态栏显示摄像机位置
`Cam: (x,y)` 和存活细胞数。编辑模式下光标可以在无界世界里自由移动，视口会跟随它。
从无限切回有界世界时，会把当前视口作为新的有限画布。
